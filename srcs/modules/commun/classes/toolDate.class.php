...
class ToolDate {
	...
	*/
	...
	static function getAddIntervalDateByIntervalDay($_zDate, $_iInterval){
		$oDBW	  = jDb::getDbWidget() ;
		$oDate = $oDBW->fetchFirst("SELECT DATE_ADD('".$_zDate."', INTERVAL ".$_iInterval." DAY) AS zDate"); 
		return $oDate->zDate;
	}	
	static function getSubIntervalDateByIntervalDay($_zDate, $_iInterval){
		$oDBW	  = jDb::getDbWidget() ;
		$oDate = $oDBW->fetchFirst("SELECT DATE_SUB('".$_zDate."', INTERVAL ".$_iInterval." DAY) AS zDate"); 
		return $oDate->zDate;
	}	
	static function getAddIntervalDateByIntervalMonth($_zDate, $_iInterval){
		$oDBW	  = jDb::getDbWidget() ;
		$oDate = $oDBW->fetchFirst("SELECT DATE_SUB('".$_zDate."', INTERVAL ".$_iInterval." MONTH) AS zDate"); 
		return $oDate->zDate;
	}	
	...
	static function getDatePlus2days($_zDate){
		$zSql = "SELECT DATE_FORMAT(DATE_ADD('".$_zDate."', INTERVAL 2 DAY), '%Y-%m-%d') AS jourSuiv"; 	
		$oDBW	  = jDb::getDbWidget() ;
		$toResults = $oDBW->fetchAll($zSql) ;
		return $toResults[0]->jourSuiv;		 
	}
	...
	static function dateAddDay ($d, $zInterval){
		$zSql	= "SELECT DATE_ADD('".$d."', INTERVAL ".$zInterval." DAY) AS d"; 	
		$oDBW	= jDb::getDbWidget() ;
		$oDate	= $oDBW->fetchFirst($zSql); 
		return $oDate->d ; 
	}
	...
	static function getNextDateDispo ($zDateHeureDebutEn){
		$zSql = "SELECT DATE_ADD('".$zDateHeureDebutEn."', INTERVAL 20 MINUTE) AS zDateNext" ;
		$oDBW	= jDb::getDbWidget() ;
		$oDate	= $oDBW->fetchFirst($zSql); 
		return $oDate->zDateNext ;
	}

	static function getDayName ($d){
		$zSql	= "SELECT DAYNAME('".$d."') AS dayName"; 	
		$oDBW	= jDb::getDbWidget() ;
		$oDate	= $oDBW->fetchFirst($zSql); 
		return trim(strtoupper($oDate->dayName)) ; 
	}

	static function gettTimeListTempPlage ($debut_matin, $fin_matin, $debut_apres_midi, $fin_soir, $zTable='ztempplage3'){
		$toH1 =  $toH2 =  $toH = array () ;

		$zSql1 = "SELECT DATE_FORMAT(heure, '%H:%i') as h
		FROM ".$zTable."
		WHERE 1=1";
		if ($debut_matin!='' && $fin_matin!=''){
			$zSql1 .= " AND (heure <= DATE_FORMAT('0000-00-00 ".$debut_matin."', '%Y-%m-%d %H:%i:%s')
			OR heure >= DATE_FORMAT('0000-00-00 ".$fin_matin."', '%Y-%m-%d %H:%i:%s')) ";
		}
		$zSql1 .= " AND am = 1" ;

		$oDBW	  = jDb::getDbWidget() ;
		$toH1 = $oDBW->fetchAll($zSql1); 

		$zSql2 = "SELECT DATE_FORMAT(heure, '%H:%i') as h
		FROM ".$zTable."
		WHERE 1=1 ";
		if ($debut_apres_midi!='' && $fin_soir!=''){
			$zSql2 .= " AND (heure <= DATE_FORMAT('0000-00-00 ".$debut_apres_midi."', '%Y-%m-%d %H:%i:%s')
			OR heure >= DATE_FORMAT('0000-00-00 ".$fin_soir."', '%Y-%m-%d %H:%i:%s')) " ;
		}

		$zSql2 .= " AND am = 0" ;
		$oDBW	  = jDb::getDbWidget() ;
		$toH2 = $oDBW->fetchAll($zSql2); 

		if (sizeof($toH1) > 0){
			foreach ($toH1 as $oH1){
				//if (in_array ($oH1->h, $toH)) {
					array_push ($toH, $oH1->h) ;
				//}
			}
		}
		if (sizeof($toH2) > 0){
			foreach ($toH2 as $oH2){
				//if (in_array ($oH2->h, $toH)) {
					array_push ($toH, $oH2->h) ;
				//}
			}
		}
		return $toH;
	}
	static function gettTimeListTempPlage1 ($debut, $fin, $zTable='ztempplage3'){
		$toH1 =  $toH = array () ;
		$zSql1 = "SELECT DATE_FORMAT(heure, '%H:%i') as h
		FROM ".$zTable."
		WHERE 1=1";
		if ($debut!='' && $fin!=''){
			$zSql1 .= " AND heure BETWEEN '0000-00-00 ".$debut."' AND '0000-00-00 ".$fin."' " ;
		}
		$oDBW	  = jDb::getDbWidget() ;
		$toH1 = $oDBW->fetchAll($zSql1); 
		if (sizeof($toH1) > 0){
