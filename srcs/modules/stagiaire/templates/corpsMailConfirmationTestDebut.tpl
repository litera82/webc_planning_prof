        ...
        <td width="620" height="418" colspan="4" valign="top" bgcolor="#FFFFFF">
        	<p style="padding:15px 40px; margin:0; text-align:justify;">
                <font face="Verdana, Geneva, sans-serif" size="2">
					...
					Votre premier cours de stage est programmé pour le {$zDateFr} à {$zHeure} avec {$zCiviliteProf} {$oUtilisateur->utilisateur_zPrenom} {$oUtilisateur->utilisateur_zNom},{if isset ($zTelResa) && $zTelResa != ''} elle vous appellera au {$zTelResa}<br />{/if}
					<br /><br /><br />	
					<i>Your first training is planned on {$zDateEn} at {$zHeure} with {$zCiviliteProf} {$oUtilisateur->utilisateur_zPrenom} {$oUtilisateur->utilisateur_zNom},{if isset ($zTelResa) && $zTelResa != ''} who will call you at {$zTelResa}<br />{/if}</i>
				</font>
				<br /><br />
				<font face="Verdana, Geneva, sans-serif" size="2">
