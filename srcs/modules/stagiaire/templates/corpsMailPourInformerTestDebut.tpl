        ...
        <td width="620" height="418" colspan="4" valign="top" bgcolor="#FFFFFF">
        	<p style="padding:15px 40px; margin:0; text-align:justify;">
                <font face="Verdana, Geneva, sans-serif" size="2">
					...
					Pour votre information, {$zCivilite} {$oClient->client_zNom} a fait une reservation pour son premier cours de stage{if isset($modifTel) && $modifTel == 1} et son numero d'appel pour le cours à changer{/if}.<br /><br /><br />
					Le cours est programmé pour le {$zDate} à {$zHeure} avec {$zCiviliteProf} {$oUtilisateur->utilisateur_zPrenom} {$oUtilisateur->utilisateur_zNom},{if isset ($zTelResa) && $zTelResa != ''} son {if isset($modifTel) && $modifTel == 1}nouveau{/if} numero d'appel est {$zTelResa}<br />{/if}
                </font>
				<br /><br /><br />
                <font face="Verdana, Geneva, sans-serif" size="2">
