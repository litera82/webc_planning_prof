...
class contentZone extends jZone {
    ...
    protected function _prepareTpl(){
		...
		if ($iPart == 0){
			...
			$duration = new jDuration(array('day'=>3));
			$oDate->add($duration);
			...
			$oDate->add($duration);
			$zDateDebut = $oDate->toString(jDateTime::DB_DFORMAT); 
			$duration1 = new jDuration(array('day'=>3));
			$oDate->add($duration1);
			$zDateFin = $oDate->toString(jDateTime::DB_DFORMAT); 
		}
        ...
        $oCriteria->evenement_iTypeEvenementId = ID_TYPE_EVENEMENT_COUR_TELEPHONE ;
		$oCriteria->moin72h = 1 ;
        ...
        {
			$toResults = autoEvemenementSrv::getUserTypeDisponibility(TYPE_UTILISATEUR_PROFESSEUR, ID_TYPE_EVENEMENT_COUR_DISPONIBLE_PLANNIFIE, toolDate::getDateSuiv($zDateDebut), $zDateFin, $oCurrentUser) ;
        }
