...
class importStagiaire
{
    ...
    static function sendMailStagiaire ($oClient)
    {
			...
			$tpl = $tplMail->fetch ('stagiaire~mailAutoPlanification') ;
	...
	static function sendMailReservation ($_tEvent, $m=0){
		jClasses::inc('client~clientSrv');
		...
		if (isset ($_tEvent['evenement_iStagiaire']) && intval($_tEvent['evenement_iStagiaire']) > 0){
			...
			$zUrlModif = "http://" . $_SERVER["HTTP_HOST"] . jUrl::get('stagiaire~default:stagiaire') . '&x=' . $oClient->client_zLogin . '&y=' . $oClient->client_zPass;
			...
			$tpl = $tplMail->fetch ('stagiaire~corpsMailConfirmationTestDebut') ;

			if ($m == 1){
				$zSujet = "Confirmation de la modification de votre reservation de votre premier cours de stage / Confirmation of modifying your booking your first training";
			}else{
				$zSujet = "Confirmation de votre reservation de votre premier cours de stage / Confirmation of your booking your first training";
			}

			mailSrv::envoiEmail (SENDER_MAIL, NAME_SENDER, $oClient->client_zMail, $oClient->client_zNom .' '.$oClient->client_zPrenom , $zSujet, $tpl,  NULL, NULL, true, NULL, NULL, NULL, array(MAIL_AUTOCOURS_COPIEDESRESERVATIONSWEBCALENDAR, $oUtilisateur->utilisateur_zMail)) ;
		}
	}
	...
	static function sendMailReservationModifContact ($iEventId){
		...
		if (isset ($_tEvent[0]->evenement_iStagiaire) && intval($_tEvent[0]->evenement_iStagiaire) > 0){
			...
			$zSujet = "Modification du numero de téléphone - Confirmation de votre reservation de votre premier cours/ Changing the phone number - Confirmation of your booking your first lesson";

			mailSrv::envoiEmail (SENDER_MAIL, NAME_SENDER, $oClient->client_zMail, $oClient->client_zNom .' '.$oClient->client_zPrenom , $zSujet, $tpl,  NULL, NULL, true, NULL, NULL, NULL, array(MAIL_AUTOCOURS_COPIEDESRESERVATIONSWEBCALENDAR, $oUtilisateur->utilisateur_zMail)) ;
		}
	}
	
	static function sendMailPourInformerReservation ($_tEvent, $m=0){
		jClasses::inc('client~clientSrv');
		jClasses::inc('client~societeSrv');
		...
		if (isset ($_tEvent['evenement_iStagiaire']) && intval($_tEvent['evenement_iStagiaire']) > 0){
			...
			$tpl = $tplMail->fetch ('stagiaire~corpsMailPourInformerTestDebut') ;
			...
			if ($m == 1){
				$zSujet = "Modification de la réservation de " . $zCivilite . " " . addslashes($oClient->client_zNom) . " " . addslashes($oClient->client_zPrenom) . " - " . addslashes($oSociete->societe_zNom). " pour un cours prévu le " . $zDate . " à "  . $zHeure;
			}else{
				$zSujet = "Réservation de " . $zCivilite . " " . addslashes($oClient->client_zNom) . " " . addslashes($oClient->client_zPrenom) . " - " . addslashes($oSociete->societe_zNom). " pour un cours prévu le " . $zDate . " à "  . $zHeure;
			}
			$zToMail = array (MAIL_TESTORALDEBUT_PROPOSITION, MAIL_TESTORALDEBUT_PROPOSITION_2);
	...
	static function sendMailPourInformerReservationModifContact ($iEventId){
		...
		if (isset ($_tEvent[0]->evenement_iStagiaire) && intval($_tEvent[0]->evenement_iStagiaire) > 0){
			...
			$zSujet = "Modification du numero de téléphone - Réservation de " . $zCivilite . " " . addslashes($oClient->client_zNom) . " " . addslashes($oClient->client_zPrenom) . " - " . addslashes($oSociete->societe_zNom). " pour un cours prévu le " . $zDate . " à "  . $zHeure;
