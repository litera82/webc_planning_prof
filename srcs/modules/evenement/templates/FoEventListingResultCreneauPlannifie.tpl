...
$(function(){ 
	...
	$('.libererCreneauPlannifie').click(
		function (){
			 if (confirm('Etes vous sûr de vouloir liberer ce créneau ?')){
				var eventId = $(this).attr('eventId'); 
				if (eventId > 0){
					$.getJSON(j_basepath + "index.php", {module:"evenement", action:"FoEvenement:libererCreneauPlannifie", eventId:eventId}, function(row){
		...
		<div class="content">
			<div class="formevent clear" style="width:960px;padding: 5px 5px 5px;">
				<form id="edit_form" action="#" method="POST" enctype="multipart/form-data" tmt:validate="true">
					<input type="hidden" name="action2" id="action2" value="{jurl 'evenement~FoEvenement:getEventListingCreneauPlannifie', array(), false}"/>
					...
					<input type="hidden" name="iClientId" id="iClientId" value="{$iClientId}"/>
						...
						<div>
							<h3 style="font-size:1.2em;">
								<a href="#" style="color:{$oEvent->typeevenements_zCouleur}">
									Cours téléphone plannifié - {$oEvent->client_zNom} {$oEvent->client_zPrenom} - {if $oUtilisateur->utilisateur_iTypeId == TYPE_UTILISATEUR_ADLINISTRATEUR}{$oEvent->utilisateur_zPrenom}&nbsp;{$oEvent->utilisateur_zPrenom}&nbsp;- &nbsp;{/if}{$oEvent->evenement_zDateJoursDeLaSemaine}&nbsp;{$oEvent->evenement_zDateHeureDebut|date_format:'%d/%m/%Y'} à {$oEvent->evenement_zDateHeureDebut|date_format:'%H:%M'}
								</a>
							</h3>
							<div>
								<p>
								  {$oEvent->typeevenements_zLibelle} plannifié</strong>
									<br />
									<br />
									...
									{/if}
									<b>Numero Individu : </b>{$oEvent->client_iNumIndividu}<br />
									<b>Nom : </b>{$oEvent->client_zNom} {$oEvent->client_zPrenom}<br />
									<b>Fontion : </b>{$oEvent->client_zFonction}<br />
									<b>Mail : </b>{$oEvent->client_zMail}<br />
									<b>Contact : </b>{$oEvent->evenement_zContactTel}<br />
									<b>Société : </b>{$oEvent->societe_zNom}
								</p>
								<button class="libererCreneauPlannifie" eventId="{$oEvent->evenement_id}" href="#">Libérer le créneau</button>
							</div>
						</div>
					{else}
						<div>
							<h3 style="font-size:1em;">
								<a href="#" style="color:{$oEvent->typeevenements_zCouleur}">
									Diponible - {if $oUtilisateur->utilisateur_iTypeId == TYPE_UTILISATEUR_ADLINISTRATEUR}{$oEvent->utilisateur_zPrenom}&nbsp;{$oEvent->utilisateur_zPrenom}&nbsp;- &nbsp;{/if}{$oEvent->evenement_zDateJoursDeLaSemaine}&nbsp;{$oEvent->evenement_zDateHeureDebut|date_format:'%d/%m/%Y'} à {$oEvent->evenement_zDateHeureDebut|date_format:'%H:%M'}
								</a>
							</h3>
							<div>
								<p>
								...
								<button class="showpopupAffestation" eventId="{$oEvent->evenement_id}" clientId="{$toParams[0]->iClientId}" dateEvent="{$oEvent->evenement_zDateHeureDebut|date_format:'%d/%m/%Y'}" heureEvent="{$oEvent->evenement_zDateHeureDebut|date_format:'%H:%M'}" href="#">Affecter à un stagiaire</button>
							</div>
						</div>
					{/if}
				{/foreach}
...
<div class="pop-up formevent clear"  id="popupAffestation" style="background-color:#E9E9E9; border:1px solid #E1E1E1; width:600px">
	<form id="edit_form" name="edit_form" action="{jurl 'evenement~FoEvenement:saveAffectationCoursPlannifie', array(), false}" method="POST" enctype="multipart/form-data" tmt:validate="true">
	<input type="hidden" name="evenement_id" id="evenement_id" value="" />
	...
	<p class="clear">
		...
		<input type="hidden" name="evenement_iStagiaire" id="evenement_iStagiaire" value="{$oClient->client_id}" />
		<b>{$oClient->client_iNumIndividu} - {$oClient->client_zNom} {$oClient->client_zPrenom}</b>
	</p>
	<p class="rdv clear">
