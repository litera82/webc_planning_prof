					...
					$.getJSON(j_basepath + "index.php", {module:"client", action:"FoClient:chargeParIdAndComposantCours", iStagiaireId:$('#evenement_iStagiaire').val()}, function(datas){
						if (datas.length >= 1){
							...
							for (var i=0; i<datas.length;i++){
								...
								if (row['NUMERO'] > 0){
									var solde = parseFloat(row['HEURES_PREVUES'].toString()) - parseFloat(row['HEURES_PRODUITES'].toString()) ;
									if (solde <= 0){
										$('#evenement_iStagiaire').val(0);		
										alert("Le stagiaire n'a plus de solde de cours!!!");
									}
									$("#evenement_solde").val(solde) ;
									$("#evenement_prevu").val(row['HEURES_PREVUES']) ;
									$("#evenement_produit").val(row['HEURES_PRODUITES']) ;
									zContent += "<tr><td style='text-align:center;background-color:#899EB0;'>"+row['Date_max_validation_format']+"</td><td style='text-align:center;background-color:#899EB0;'>"+solde+"</td><td style='text-align:center;background-color:#899EB0;'>"+row['HEURES_PREVUES']+"</td><td style='text-align:center;background-color:#899EB0;'>"+row['HEURES_PRODUITES']+"</td><td style='text-align:center;background-color:#899EB0;'>"+row['INDIVIDUSNOMFAMILLE']+" "+row['INDIVIDUSPRENOM'] + "&nbsp;&nbsp;<a href='#' title='Information sur le composant du cours' id='infoComposant' NUMERO='"+row['NUMERO']+"' CODE_STAGIAIRE_MIRACLE='"+row['CODE_STAGIAIRE_MIRACLE']+"' STAGE='"+row['STAGE']+"' STAGESOCIETE='"+row['STAGESOCIETE']+"' STAGELANGUE='"+row['STAGELANGUE']+"' NIVEAU='"+row['NIVEAU']+"' INDIVIDUS_1NOMFAMILLE='"+row['INDIVIDUS_1NOMFAMILLE']+"' INDIVIDUS_1PRENOM='"+row['INDIVIDUS_1PRENOM']+"' COMPTEUR='"+row['COMPTEUR']+"' REFPRODUIT='"+row['REFPRODUIT']+"' TYPEPRODUCTION='"+row['TYPEPRODUCTION']+"' PROF_NO='"+row['PROF_NO']+"' INDIVIDUSNOMFAMILLE='"+row['INDIVIDUSNOMFAMILLE']+"' INDIVIDUSPRENOM='"+row['INDIVIDUSPRENOM']+"' DATEAFFECTATION='"+row['DATEAFFECTATION']+"' HEURES_PREVUES='"+row['HEURES_PREVUES']+"' LIBELLESTAGIAIRE='"+row['LIBELLESTAGIAIRE']+"' QTE_PRODUITES='"+row['QTE_PRODUITES']+"' HEURES_PRODUITES='"+row['HEURES_PRODUITES']+"' DATE_TRANSFERT_ACCESS='"+row['DATE_TRANSFERT_ACCESS']+"' DATE_TRANSFERT_WEBCAL='"+row['DATE_TRANSFERT_WEBCAL']+"' class='infoComposant'><img src='{/literal}{$j_basepath}{literal}design/front/images/design/pictos/information.png' alt='Information sur le composant du cours' /></a></td></tr>";
									
								}
							}
							if (zContent != ""){
//<th style='text-align:center;background-color:#2C2C2C;color:#FFFFFF;'>&nbsp;</th>
								$('.contentComposant').empty();
