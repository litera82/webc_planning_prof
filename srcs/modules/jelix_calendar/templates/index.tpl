...
<script type="text/javascript">
	$(function(){ 
		...
		$("a.project").simpletooltip({ click: true, showEffect: "slideDown", hideEffect: "slideUp", hideOnLeave: true,
			customTooltip: function(target){
				...
				var zurlCreneauPlannifie = "";
				if ($(target).attr("urlCreneauPlannifie") != ""){
					zurlCreneauPlannifie = '<tr><td class="left" style="width:auto">&nbsp;</td><td class="right"><a target="_blank" href="'+$(target).attr("urlCreneauPlannifie")+'" style="color:#0000FF;font-weight:bold;">Planification</a></td></tr>';
				}
				var vPrevu=$(target).attr("prevu") ;
				var vSolde=$(target).attr("solde") ;
				var vProduit=$(target).attr("produit") ;
				if ($(target).attr("clientsolde_prevu") !== undefined){vPrevu = $(target).attr("clientsolde_prevu") ;}
				if ($(target).attr("clientsolde_solde") !== undefined){vSolde = $(target).attr("clientsolde_solde") ;}
				if ($(target).attr("clientsolde_produit") !== undefined){vProduit = $(target).attr("clientsolde_produit") ;}
	...
	function rechercherStagiaire() {
		if ($('.input_zStagiaire').val() != '' && $('.input_zStagiaire').val().length > 2){
			$.getJSON(j_basepath + "index.php", {module:"evenement", action:"FoEvenement:autocompleteStagiaire", q:$('.input_zStagiaire').val()}, function(row){
				html = '';
				var selected = '';
				if (row.length > 0)
				{
					...
					for (i=0 ; i<row.length ; i++)
					{	
						html += '<option ondblclick="getClientComposantCours('+row[i].client_id+')" value="'+ row[i].client_id +'" '+ selected +'>'+ row[i].client_zNom + ' ' + row[i].client_zPrenom +'&nbsp;&nbsp;[' + row[i].client_zTel + ']'+'&nbsp;&nbsp;[' + row[i].societe_zNom + ']'+'&nbsp;&nbsp;[' + row[i].client_zVille + ']' +'</option>';
					}
	...
	function getClient (id){
        ...
        function getClientComposantCours (id){
		$.getJSON(j_basepath + "index.php", {module:"client", action:"FoClient:chargeParIdAndComposantCours", iStagiaireId:id}, function(datas){
                        if (datas.length >= 1){
                                var zContent = "" ;
                                var checked = "" ;
                                for (var i=0; i<datas.length;i++){
                                        ...
                                        if (row['NUMERO'] > 0){
                                                ...
                                                if (solde <= 0){
                                                        alert("Le stagiaire que vous avez choisi n'a plus de solde de cours!!!\n\nDate max validation : "+row['Date_max_validation_format']+"\nSolde avant saisie : "+solde+"\nHeures prevues : "+row['clientsolde_prevu']+"\nCours produit : "+row['clientsolde_produit']+"\nProfesseur : "+row['INDIVIDUSNOMFAMILLE']+" "+row['INDIVIDUSPRENOM']+"\n");
                                                }else{
                                                    $('#iStagiaire').val(row["client_id"]);
                                                    $('.input_zStagiaire').val(row["client_zNom"]+' '+row["client_zPrenom"]);
                                                    $('.affichageListeStagiaire').hide();
                                                }
                                        }else{
                                            $('#iStagiaire').val(row["client_id"]);
