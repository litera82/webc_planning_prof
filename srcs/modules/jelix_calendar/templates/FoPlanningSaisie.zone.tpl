				...
				<tr class="busy25">
					...
					{if $oUtilisateur->utilisateur_plageHoraireId == 2}
						{if $oTimeListeDemiHeure->time1 == '07:30' || $oTimeListeDemiHeure->time1 == '08:30' || $oTimeListeDemiHeure->time1 == '09:30' || $oTimeListeDemiHeure->time1 == '10:30' || $oTimeListeDemiHeure->time1 == '11:30' || $oTimeListeDemiHeure->time1 == '12:30' || $oTimeListeDemiHeure->time1 == '13:30' || $oTimeListeDemiHeure->time1 == '14:30' || $oTimeListeDemiHeure->time1 == '15:30' || $oTimeListeDemiHeure->time1 == '16:30' || $oTimeListeDemiHeure->time1 == '17:30' || $oTimeListeDemiHeure->time1 == '18:30' || $oTimeListeDemiHeure->time1 == '19:30' || $oTimeListeDemiHeure->time1 == '20:30' || $oTimeListeDemiHeure->time1 == '21:30'}
							<td style="border-bottom:1px solid #6C6C6C;" id="cell_{$oDateListe->zDate|replace:'-':'_'}_{$oTimeListeDemiHeure->time1|replace:':':'_'}">
						{else}
							<td style="border-bottom:1px solid #DCDCDC;" id="cell_{$oDateListe->zDate|replace:'-':'_'}_{$oTimeListeDemiHeure->time1|replace:':':'_'}">
						{/if}
					{else}
					...
					{/if}
						<div class="clear divajouterEvent">
							...
							<a class="actionEventHebdo" id="actionEvent" title="Ajouter un évènement rapidement" typeEvent="" iUtilisateurId="{$userId}" href="#" style="cursor:pointer;" zDate="{$oDateListe->zDate|date_format:'%d/%m/%Y'}" iTime="{$oTimeListeDemiHeure->time1}">
								...
								{foreach $toEventUser as $oEventUser}
									...
									<input type="hidden" name="urlDeleteEvent" id="urlDeleteEvent" value="{jurl 'evenement~FoEvenement:deleteEvent', array('date'=>$date)}" />
										<ul class="conge" style="width:146px">
											<a class="commentEventHebdo" id="commentEvent" title="Description de l'événement" href="#tooltip" colorbg="{$oEventUser->typeevenements_zCouleur}" description="{$oEventUser->evenement_zDescription}" style="cursor:pointer;">
											...
											<a class="deleteEventHebdo" id="deleteEvent" title="Supprimer l'événement" onclick="deleteEvent({$oEventUser->evenement_id});" style="cursor:pointer;">
