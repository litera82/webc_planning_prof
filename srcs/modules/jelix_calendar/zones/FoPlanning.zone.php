...
class FoPlanningZone extends jZone 
{
 
    protected $_tplname		= 'jelix_calendar~FoPlanning.zone' ;
	...
	*/
	protected function _prepareTpl()
	{
		...
		foreach($toEventUser['toListes'] as $oEvent){
			...
			$toParams = array ();
			$toParams[0] = new StdClass ();
			$toParams[0]->eventId = $oEvent->evenement_id;
			
			$toEnv = clientsenvironnementSrv::listCriteria($toParams);

			if (sizeof($toEnv['iResTotal']) > 0 && isset($toEnv['toListes'][0])){
				$oEvent->eventId = $toEnv['toListes'][0]->eventId; 
				$oEvent->bureau = $toEnv['toListes'][0]->bureau; 
				$oEvent->navigateur = $toEnv['toListes'][0]->navigateur; 
				$oEvent->telFixe = $toEnv['toListes'][0]->telFixe; 
				$oEvent->telMobile = $toEnv['toListes'][0]->telMobile; 
				$oEvent->skype = $toEnv['toListes'][0]->skype; 
				$oEvent->casqueSkype = $toEnv['toListes'][0]->casqueSkype; 
			}else{
				$oEvent->eventId = 0; 
				$oEvent->bureau = 0; 
				$oEvent->navigateur = 0; 
				...
				case 2: // 30 Minutes
					...
					if ($oEvent->original == 1){
						...
						for ($i=0; $i<$iNombreD -1; $i++){
							...
							// Verifier si la plage est libre 
							$iEventExist = evenementSrv::getNextEvent($oEvent->evenement_zDateHeureDebut);
							if ($iEventExist == 0){
		...
		$toRessources = utilisateursSrv::listCriteria($toCriteria);*/
		...
		//print_r($toEventUser['toListes']);die;
	}
}
?>