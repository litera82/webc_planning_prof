    ...
    public function beforeAction ($params){
        ...
        /*if($needAuth){
            if($notLogged){
				...
				$auth = $gJConfig->coordplugins['auth'];
				$authok= true;    
				if ($auth == 'fo.auth.coord.ini.php'){
					$bExist = false;
					$user = $_SESSION[$this->config['session_name']];				
					if (isset($params['droitsActions.profilAutorises'])){
						if(in_array($user->compte_typeCompteId, $params['droitsActions.profilAutorises'])){
								$bExist = true;
						}
						if(!$bExist) {
							$selector = new jSelectorAct('accueil~accueilFo:index');
						}
					}
				}else{
					jClasses::inc('nacl2~nAclSrv');
					$dao = jDao::get('nacl2~nacl_right');
					//Chargement de la liste de(s) action(s) autorisé du profil (administrateur)
					$toAction = $dao->getBy(null, 1, null, null, null, array($_SESSION['PROFIL_CODE']), false);

					$toAllAction = $toAction->fetchAll(); 

					$bExist = false;
					foreach ($toAllAction as $oAllAction){
						if(in_array($oAllAction->right_actionId, $params['droitsActions.profilAutorises'])){
							$bExist = true;
							break;
						}
					}
					if(!$bExist) {
						$selector = new jSelectorAct('commun~authentification:actionNonAutorisee');
					}
				}
